...
def launch_setup(context, *args, **kwargs):
    ...
    # Absolute path to combined controllers YAML (owned by this package)
    controllers_yaml = PathJoinSubstitution(
        [FindPackageShare("ur5_l201"), "config", "ur5_robotiq_controllers.yaml"]
    )

    # Combined URDF: UR5 + adapter plate + Robotiq 2F-85
    robot_description_content = Command(
        [
            PathJoinSubstitution([FindExecutable(name="xacro")]),
            " ",
            PathJoinSubstitution(
                [FindPackageShare("ur5_l201"), "urdf", "ur5_robotiq_2f85.urdf.xacro"]
            ),
            " ", "name:=ur",
            " ", "ur_type:=",             ur_type,
            " ", "tf_prefix:=",           tf_prefix,
    ...
    gz_spawn_entity = Node(
        ...
        arguments=[
            ...
            "-x", "0.0",
            "-y", "0.0",
            "-z", "0.63",   # on top of the lab table
            "-R", "0.0",
            "-P", "0.0",
            "-Y", "0.0",
        ],
